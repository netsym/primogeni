#!/bin/sh
#Exogeni: become a root in your local machine and then ssh to exogeni slvier: root@mylocalpc:~# ssh -i ~/.ssh/id_geni_ssh_rsa 131.94.144.48
#change the permission of this script as: chmod +x insall_primogeni.sh
#Calling this script: ./install_primogeni.sh local/image  directory_if_local
# local/image: (local)local installation of primogeni or (image)making image at exogeni/instageni/protogeni
# VARIABLES:
# 
# MPICHVAR=1.3.2p1
# HYDRAVAR=/root/Downloads/hydra-1.3.2p1
#

#rm -rf ~/Downloads; rm -rf /primex; rm -rf /usr/local/jdk1.6.0_23; rm -rf /usr/mpich;
