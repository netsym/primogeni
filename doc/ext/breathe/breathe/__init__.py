...
class DoxygenClassDirective(BaseDirective):
    ...
    def run(self):
        ...
        try:
            ...
            warning = ('doxygen%s: nate0 Cannot find %s "%s" in doxygen xml output for project "%s" from directory: %s'
...
class DoxygenInterfaceDirective(BaseDirective):
    ...
    def run(self):
        ...
        try:
            ...
            warning = ('doxygen%s: nate1 Cannot find %s "%s" in doxygen xml output for project "%s" from directory: %s'
                    % (self.kind, self.kind, name, project_info.name(), project_info.path()))
            return [docutils.nodes.warning("", docutils.nodes.paragraph("", "", docutils.nodes.Text(warning))),
                    self.state.document.reporter.warning(warning, line=self.lineno)]
