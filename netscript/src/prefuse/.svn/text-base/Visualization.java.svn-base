 ...
 * structure (i.e., {@link prefuse.data.ITable}, {@link prefuse.data.Graph},
 ...
 * {@link prefuse.data.ITable}, {@link prefuse.data.Graph}, or
...
public class Visualization {
            ...
            String group, TupleSet data, Predicate filter)
    {
        if ( data instanceof ITable ) {
            return addITable(group, (ITable)data, filter);
     ...
     */
    public synchronized VisualTable addITable(String group) {
     ...
     * @return the added VisualTable
     */
    public synchronized VisualTable addITable(String group, Schema schema) {
     ...
     */
    public synchronized VisualTable addITable(String group, ITable table) {
        return addITable(group, table, (Predicate)null);
    }
     ...
     */
    public synchronized VisualTable addITable(
            String group, ITable table, Predicate filter)
    {
     ...
     */
    public synchronized VisualTable addITable(
            String group, ITable table, Schema schema)
    {
        return addITable(group, table, null, schema);
    }
     ...
     */
    public synchronized VisualTable addITable(
            String group, ITable table, Predicate filter, Schema schema)
    {
     ...
     * @return the added VisualTable
     */
    public synchronized VisualTable addITable(VisualTable table) {
    	addDataGroup(table.getGroup(), table, table.getParentITable());
            ...
            Predicate filter, Schema nodeSchema, Schema edgeSchema)
    {
        ...
        VisualTable nt, et;
        nt = addITable(ngroup, graph.getNodeITable(), filter, nodeSchema);
        et = addITable(egroup, graph.getEdgeITable(), filter, edgeSchema);
            ...
            Predicate filter, Schema nodeSchema, Schema edgeSchema)
    {
        ...
        VisualTable nt, et;
        nt = addITable(ngroup, tree.getNodeITable(), filter, nodeSchema);
        et = addITable(egroup, tree.getEdgeITable(), filter, edgeSchema);
    ...
    // -- Derived ITables and Decorators ---------------------------------------
     ...
     */
    public synchronized VisualTable addDerivedITable(
     ...
     * {@link #addDerivedITable(String, String, Predicate, Schema)} method,
    ...
    {
        VisualTable t = addDerivedITable(group,source,filter,VisualItem.SCHEMA);
    ...
    {
        VisualTable t = addDerivedITable(group, source, filter, schema);
        t.setTupleManager(new TupleManager(t, null, TableDecoratorItem.class));
        return t;
    }
    ...
    public VisualItem getVisualItem(String group, Tuple t) {
        ...
        if ( ts instanceof VisualTable ) {
            ...
            Graph g = (Graph)ts;
            vt = (VisualTable)(t instanceof Node ? g.getNodeITable() 
                                                 : g.getEdgeITable());
        } else {
            return null;
        }
