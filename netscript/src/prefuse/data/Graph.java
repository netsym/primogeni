 ...
 * </p>
 * 
 * <p>Both nodes and edges are represented by backing data {@link ITable}
...
public class Graph extends CompositeTupleSet {
    ...
    /** ITable containing the adjacency lists for the graph */
    protected ITable m_links;
     ...
     * @param directed true for directed edges, false for undirected
     */
    public Graph(ITable nodes, boolean directed) {
     ...
     * table
     */
    public Graph(ITable nodes, boolean directed, String nodeKey,
            String sourceKey, String targetKey) {
        ITable edges = new Table();
     ...
     * @param directed true for directed edges, false for undirected
     */
    public Graph(ITable nodes, ITable edges, boolean directed) {
     ...
     * table
     */
    protected void init(ITable nodes, ITable edges, boolean directed, 
            String nodeKey, String sourceKey, String targetKey)
    {
        // sanity check
        if ( (nodeKey!=null && 
        ...
        super.addSet(EDGES, edges.getTableToListenTo());
        super.addSet(NODES, nodes.getTableToListenTo());
        ...
        m_listener.setEdgeITable(edges);
    }
     ...
     */
    public void dispose() {
        getNodeITable().removeTableListener(m_listener);
        getEdgeITable().removeTableListener(m_listener);
    }
     ...
     */
    public void setEdgeITable(ITable edges) {
        ITable oldEdges = getEdgeITable();
        oldEdges.removeTableListener(m_listener);
        m_edgeTuples.invalidateAll();
        m_links.clear();
        
        init(getNodeITable(), edges, m_directed, m_nkey, m_skey, m_tkey);
    }
     ...
     */
    protected void initLinkITable() {
        // set up cache of node data
        m_links = createLinkITable();
                
        IntIterator edges = getEdgeITable().rows();
        while ( edges.hasNext() ) {
     ...
     */
    protected ITable createLinkITable() {
        return LINKS_SCHEMA.instantiate(getNodeITable().getMaximumRow()+1);
    }
    ...
    protected void updateDegrees(int e, int incr) {
        if ( !getEdgeITable().isValidRow(e) ) return;
    ...
    public long getKey(int node) {
        return m_nkey == null ? node : getNodeITable().getLong(node, m_nkey);
    }
    ...
    public boolean removeNode(int node) {
        ITable nodeITable = getNodeITable();
        if ( nodeITable.isValidRow(node) ) {
        ...
        return nodeITable.removeRow(node);
    }
    ...
    public boolean removeEdge(int edge) {
        return getEdgeITable().removeRow(edge);
    }
     ...
     */
    public ITable getNodeITable() {
        return (ITable)getSet(NODES);
    }
    ...
    public int getNodeCount() {
        return getNodeITable().getRowCount();
    }
     ...
     */
    public ITable getEdgeITable() {
        return (ITable)getSet(EDGES);
    }
    /**
    ...
    public int getEdgeCount() {
        return getEdgeITable().getRowCount();
    }
    ...
    public int getSourceNode(int edge) {
        return getNodeIndex(getEdgeITable().getLong(edge, m_skey));
    }
    ...
    public int getTargetNode(int edge) {
        return getNodeIndex(getEdgeITable().getLong(edge, m_tkey));
    }
    ...
    public Iterator tuples(Predicate filter) {
        if ( filter == null ) {
            ...
            return new CompositeIterator(
                    m_edgeTuples.iterator(getEdgeITable().rows(filter)),
                    m_nodeTuples.iterator(getNodeITable().rows(filter)));
        }
    }
     ...
     */
    protected void fireGraphEvent(ITable t, 
            int first, int last, int col, int type)
    {
        String table = (t==getNodeITable() ? NODES : EDGES);
    ...
    protected class Listener implements TableListener, ColumnListener {
        ...
        public void tableChanged(ITable t, int start, int end, int col, int type) {
            if ( !containsSet(t) )
