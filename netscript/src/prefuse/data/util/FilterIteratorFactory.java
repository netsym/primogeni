...
public class FilterIteratorFactory {
     ...
     * filtered by the given predicate.
     * @param t the ITable to iterate over
     * @param p the filter predicate
     * @return a filtered iterator over the table rows
     */
    public static IntIterator rows(ITable t, Predicate p) {
        // attempt to generate an optimized query plan
     ...
     * @param t the ITable to iterator over
     * @param p the filter predicate
     * @return an optimized iterator, or null if no optimization was found
     */
    protected static IntIterator getOptimizedIterator(ITable t, Predicate p) {
    ...
    protected static IntIterator getColumnIterator(
            ITable t, String field, boolean val)
    {
    ...
    protected static IntIterator getOrIterator(ITable t, OrPredicate op) {
        int size = op.size();
        if ( size > 1 ) {
    ...
    protected static IntIterator getAndIterator(ITable t, AndPredicate ap) {
    ...
    protected static IntIterator getComparisonIterator(ITable t, 
    ...
    protected static IntIterator getRangeIterator(ITable t, RangePredicate rp) {
        ColumnExpression col;
        Expression l, r;
