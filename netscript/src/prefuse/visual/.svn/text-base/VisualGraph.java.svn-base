...
public class VisualGraph extends Graph implements VisualTupleSet {
     ...
     * @see prefuse.data.Graph#fireGraphEvent(prefuse.data.ITable, int, int, int, int)
     */
            ...
            int first, int last, int col, int type)
    {
        ...
        if ( type==EventConstants.UPDATE && 
             col==VisualItem.IDX_VALIDATED && t==getNodeITable() )
        {
            VisualTable nodes = (VisualTable)t;
            VisualTable edges = (VisualTable)getEdgeITable();
...
//                }
    ...
    public int getNodeIndex(int key) {
        if ( m_nkey == null ) {
            return ((VisualTable)getNodeITable()).getChildRow(key);
        } else {
