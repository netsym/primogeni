...
public class DataLib {
     ...
     * and field. The {@link ITable#canGetDouble(String)} method must return
    ...
    public static Object[] ordinalArray(TupleSet tuples, String field,
                                        Comparator cmp)
    {
        if ( tuples instanceof ITable ) {
            ColumnMetadata md = ((ITable)tuples).getMetadata(field);
    ...
    public static Class inferType(TupleSet tuples, String field) {
        if ( tuples instanceof ITable ) {
            return ((ITable)tuples).getColumnType(field);
        } else {
