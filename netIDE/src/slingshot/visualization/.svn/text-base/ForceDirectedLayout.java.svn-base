...
public class ForceDirectedLayout extends Layout {
    ...
    private boolean m_ranonce;
    private int m_iterations = 1000;
        ...
        while ( iter.hasNext() ) {
            ...
            if ( item.isFixed() ) {
                ...
                if ( Double.isNaN(item.getX()) ) {
                    ...
                    if(node != null) {
                    	node.updateLocation(new PrefuseLocation(0.0,0.0));
                    }
                }
                continue;
            }
            ...
            final IModelNode node = (IModelNode)item.get(VizVisitor.MODELNODE);
            if(node != null) {
            	node.updateLocation(new PrefuseLocation(x, y));
            }
        }
    }
    ...
    public void reset() {
        ...
        while ( iter.hasNext() ) {
            ...
            if ( fitem != null ) {
                ...
                final IModelNode node = (IModelNode)item.get(VizVisitor.MODELNODE);
                if(node != null) {
                	final PrefuseLocation loc = node.updateLocation(null);
                	if(loc != null) {
                		fitem.location[0]=(float)loc.x;
                		fitem.location[1]=(float)loc.y;
                	}
                }
            }
        }
