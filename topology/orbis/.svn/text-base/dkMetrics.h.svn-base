...
float
clustering(const VertexListGraph& g)
{
    // Boost typedefs 
    ...
    for (typename std::map<int, int>::iterator i = degreeDistribution_int.begin();
	 i != degreeDistribution_int.end();
	 ++i)
    {
	int degree = i->first;
	int count = i->second;
	...
	for (boost::tie(ai1, aend1) = adjacent_vertices(*vi, g);
	     ai1 != aend1; ai1++)
	{
...
template<class VertexListGraph>
float
distanceMetrics(const VertexListGraph& g, 
...
// 	}
    ...
    for (std::map<int,int>::iterator i = dist_distr_int.begin();
	 i != dist_distr_int.end();
	 ++i)
    {
	distanceDistribution[i->first] = float(i->second) / float(totalNumPaths);
    }
...
template<class VertexListGraph>
float
averageDegree(const VertexListGraph& g)
{
    typedef boost::graph_traits<VertexListGraph> GraphTraits;
    typedef typename GraphTraits::vertex_descriptor vertex_descriptor;
    typedef typename GraphTraits::vertex_iterator vertex_iterator;
...
template<class VertexListGraph>
float
find_r(const VertexListGraph& g)
{
    typedef boost::graph_traits<VertexListGraph> GraphTraits;
    typedef typename GraphTraits::vertex_descriptor vertex_descriptor;
    typedef typename GraphTraits::edge_iterator edge_iterator;
...
//         ass_sq +=  ( (float) ( (k1 * k1) + (k2 * k2) ) ) / 2.0 ;
	...
	(float) ( ((ass_sum_float / edge_count)* (ass_sum_float / edge_count) ) ) ;
