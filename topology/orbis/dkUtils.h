...
template<class T>
void 
dkShuffleList(std::list<T> &theList)
{
    // O(n)
...
bool 
readInputGraph(VertexListGraph& g, const std::string& filename)
{
    ...
    typename boost::property_map<VertexListGraph, 
	boost::vertex_name_t>::type nameMap = 
	...
	if (sscanf(inLine, "%d %d", &id1, &id2) != EOF)
	{
	    if (id1 == id2)
	    {
		std::cerr  << "Self edge detected ... line #" 
			   << lineNumber << std::endl;
		continue;
	    }
...
template<class VertexListGraph>
int
printGraph(VertexListGraph& g,  std::ostream& outFile)
{    
    typedef boost::graph_traits<VertexListGraph> GraphTraits;
...
template<class VertexListGraph>
int
printGraph(VertexListGraph& g,  const std::string& filename)
{
    typedef boost::graph_traits<VertexListGraph> GraphTraits;
    boost::function_requires< boost::VertexAndEdgeListGraphConcept
	<VertexListGraph> >();    
