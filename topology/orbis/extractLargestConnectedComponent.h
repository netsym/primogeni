...
int rewireToLargestConnectedComponent1k(VertexListGraph& G)
{
    ...
    typedef std::map<vertex_descriptor, 
	vertices_size_type> VertexComponentMap;
    VertexComponentMap vertexComponentMap;
    boost::associative_property_map<VertexComponentMap>  
	vertexComponentPropertyMap(vertexComponentMap);
    
    // Colormap needed by GCC boost algorithm
    typedef std::map<vertex_descriptor, 
	boost::default_color_type> ColorMap;
    ColorMap colorMap;
    boost::associative_property_map<ColorMap> 
	colorPropertyMap(colorMap);

    unsigned int totalComponents = 
	connected_components(G, vertexComponentPropertyMap,
    ...
    // is the biggest:
    std::map<vertices_size_type, 
	std::vector<vertex_descriptor> > verticesByComponent;

    for (typename VertexComponentMap::iterator i = vertexComponentMap.begin();
	 i != vertexComponentMap.end();
	 i++)
    {
	...
	vertex_descriptor disconnectedNode = 
	    vertexList[ rand() % vertexList.size() ];

	std::cerr << "Selected disconnected node:" << disconnectedNode
		  << " degree:" << out_degree(disconnectedNode, G) 
		  << std::endl;
	...
	for (boost::tie(vi, vend) = adjacent_vertices(gccNode, G);
	     vi != vend;
	     ++vi, ++cnt)
	{
	    if (cnt >= randNum)
...
{ 
    ...
    totalComponents = connected_components(G, 
					   vertexComponentPropertyMap,
					   boost::color_map(colorPropertyMap));
...
// 	    }
	    ...
	    if (out_degree(*vi, G) > 0)
	    {
		if (oldToNewVertexMap.find(*vi) == 
		    oldToNewVertexMap.end())
		{
    ...
    for (boost::tie(edgeIter, edgeIterEnd) = edges(G);
