...
final public class ParseConfFile {
	...
	public static Model Parse(String confFile) {
		...
		try {
			while (t.ttype != t.TT_EOF) {
				...
				if (m == null) {
					Util
							.ERR(" Could not create a model from specified model configuration. ");
				}
	...
	private static TopDownHierModel ParseTopDown(HashMap params) {
		...
		float interMin = (float) ((Double) params.get("BWInterMin"))
				.doubleValue();
		float interMax = (float) ((Double) params.get("BWInterMax"))
				.doubleValue();
		float intraMin = (float) ((Double) params.get("BWIntraMin"))
				.doubleValue();
		float intraMax = (float) ((Double) params.get("BWIntraMax"))
				.doubleValue();
		
		//modified
		float duplication_threshold = (float) ((Double) params.get("Duplication"))
					      .doubleValue();
		...
		//modified
		return new TopDownHierModel(models, edgeConn, k, bwInter, interMin,
				interMax, bwIntra, intraMin, intraMax, duplication_threshold);

	}

	private static BottomUpHierModel ParseBottomUp(HashMap params) {
		Util.DEBUG("Parser found Bottom Up Model");
		int groupingMethod = (int) ((Double) params.get("Grouping"))
				.doubleValue();
		int assignType = (int) ((Double) params.get("AssignType"))
				.doubleValue();
		...
		return new BottomUpHierModel(CreateModel(p), numASNodes,
				groupingMethod, assignType, bwInter, interMin, interMax);
	}
	...
	private static FileModel ParseFileModel(HashMap params, int modelName) {
		...
		FileModel fm = new FileModel(format, file, modelName, HS, LS, bwDist,
				bwMin, bwMax);
		if (fm == null)
			Util
					.ERR("Could not create an imported file model from the configuration file. ");
		return fm;
	}
	...
	private static Model ParseWaxman(HashMap params, int modelName) {
		...
		if (modelName == ModelConstants.RT_WAXMAN) {
			...
			return new RouterWaxman(N, HS, LS, np, m, alpha, beta, gt, bwDist,
					bwMin, bwMax);
		} else if (modelName == ModelConstants.AS_WAXMAN) {
			Util.DEBUG("Parser found AS Waxman");
			return new ASWaxman(N, HS, LS, np, m, alpha, beta, gt, bwDist,
					bwMin, bwMax);
		}

		Util.ERR("Error in creating model from config file, line:" + t.lineno()
				+ " " + t.sval + "\n");
	...
	private static Model ParseBarabasi(HashMap params, int modelName) {
		...
		if (modelName == ModelConstants.RT_BARABASI) {
			Util.DEBUG("Parser found Router Barabasi");
			return new RouterBarabasiAlbert(N, HS, LS, np, m, bwDist, bwMin,
					bwMax);
		} else if (modelName == ModelConstants.AS_BARABASI) {
			Util.DEBUG("Parser found AS Barabasi");
	...
	private static Model CreateModel(HashMap params) {
		...
		if (modelName == ModelConstants.RT_WAXMAN
				|| modelName == ModelConstants.AS_WAXMAN)
			return ParseWaxman(params, modelName);
		else if (modelName == ModelConstants.RT_BARABASI
				|| modelName == ModelConstants.AS_BARABASI)
		...
		else if (modelName == ModelConstants.AS_FILE
				|| modelName == ModelConstants.RT_FILE) // Reading from
														// RouterFile
	...
	public static HashSet ParseExportFormats() {
		HashSet ExportFormat = new HashSet(2);
		...
		try {
			...
			Util.ERR("Parser Error when parsing Export Outputs at lineno: "
					+ t.lineno() + "\n", e);
		}
		return ExportFormat;
	}
