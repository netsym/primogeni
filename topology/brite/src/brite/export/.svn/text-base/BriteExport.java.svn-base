...
/**
 * Export.BriteExport provides functionality to export a topology into a BRITE
 * format file. The BRITE format looks like: <br>
 * 
 * <pre>
 ...
 * 
 * <br>
 * Please see the BRITE User Manual (at http://www.cs.bu.edu/brite/docs.htm) for
 * more information.
 */
public class BriteExport {
	...
	/**
	 * Class Constructor: Returns a BriteExport object which your code my keep
	 * around. Does not actually write the topology to the file. You must
	 * explicitly call the <code>export()</code> method of this object in order
	 * to write to the file.
	 * 
	 * @param t
	 *            the topology object to export
	 * @param outFile
	 *            the destination file to write the topology to.
	 */
	...
	public void export() {
		...
		try {
			...
			bw.write("Topology: ( " + g.getNumNodes() + " Nodes, "
					+ g.getNumEdges() + " Edges )");
			bw.newLine();
			...
			for (int i = 0; i < nodes.length; ++i) {
				...
				int nodeID = n.getID();
				String copy_id="<null>";
				if(n.getCopyOf()!=null) copy_id="<"+n.getCopyOf().getID()+">";
				if (n.getNodeConf() instanceof RouterNodeConf) {
					ASid = ((RouterNodeConf) n.getNodeConf()).getCorrAS();
					specificNodeType = ((RouterNodeConf) n.getNodeConf())
							.getType();
				}
				...
				bw.write(nodeID + "\t" + x + "\t" + y + "\t" + indegree + "\t"
						+ outdegree + "\t" + ASid+"\t"+copy_id);
			...
			for (int i = 0; i < edges.length; ++i) {
				...
				String copy_id="<null>";
				if(e.getCopyOf()!=null) copy_id="<"+e.getCopyOf().getID()+">";

				bw.write(e.getID() + "\t" + src.getID() + "\t" + dst.getID());
				bw.write("\t" + dist + "\t" + delay + "\t" + e.getBW());
				bw.write("\t" + asFrom + "\t" + asTo+"\t"+copy_id);

				if (e.getEdgeConf() instanceof ASEdgeConf) {
					int specificEdgeType = ((ASEdgeConf) e.getEdgeConf())
							.getType();
					...
					int specificEdgeType = ((RouterEdgeConf) e.getEdgeConf())
							.getType();
