...
import java.util.Random;  //i don't know why i'm including this ;)
...
final public class RandomGenManager {
    ...
    public RandomGenManager(long seed) {
		Random r = new Random(seed);
		setPlaceNodesSeed(r.nextLong());
		setConnectNodesSeed(r.nextLong());
		setBWSeed(r.nextLong());
		setEdgeConnSeed(r.nextLong());
		setGroupingSeed(r.nextLong());
		setAssignSeed(r.nextLong());
    }
    ...
    */
	...
	StreamTokenizer t = new SeedTokenizer(br);
	try {
	    t.nextToken();
	    String seedName="";
	    long seedValue=0;
	    ...
	    Util.ERR("Error parsing seedfile at lineno: "+t.lineno()+".\n",e);
	}
	...
	*/
    ...
    /*This is to maintain compatibility with erand48() and C++ version
      ...
      @param String[] An array of three strings which when conctaneated are equal to the long seed.
    ...
    */
	...
	try {
	    ...
	    bw.write("PLACES " + placeSeed + "\t# used when placing nodes on the plane");
	    bw.newLine();
	    bw.write("CONNECT "+ connectSeed + "\t# used when interconnecting nodes");
	    bw.newLine();
	    bw.write("EDGE_CONN " + ECSeed+"\t# used in the edge connection method of top down hier");
	    bw.newLine();
	    bw.write("GROUPING " +GroupSeed+"\t# used when deciding which routers to group into an AS in bottom up hier");
	    bw.newLine();
	    bw.write("ASSIGNMENT "+AssignSeed+"\t# used when deciding how many routers to group into an AS in bottom up hier");
	    bw.newLine();
	    bw.write("BANDWIDTH "+BWSeed+"\t# used when assigning bandwidths");
	    bw.newLine();
	    bw.close();
	}
	catch (IOException e) {
	...
	try {
	    ...
	    bw.write("PLACES " + PlaceRandom.nextLong() + "\t# used when placing nodes on the plane");
	    bw.newLine();
	    bw.write("CONNECT "+ ConnectRandom.nextLong() + "\t# used when interconnecting nodes");
	    bw.newLine();
	    bw.write("EDGE_CONN " + EdgeConnRandom.nextLong()+"\t# used in the edge connection method of top down hier");
	    bw.newLine();
	    bw.write("GROUPING " +GroupingRandom.nextLong()+"\t# used when deciding which routers to group into an AS in bottom up hier");
	    bw.newLine();
	    bw.write("ASSIGNMENT "+AssignRandom.nextLong()+"\t# used when deciding how many routers to group into an AS in bottom up hier");
	    bw.newLine();
	    bw.write("BANDWIDTH "+BWRandom.nextLong()+"\t# used when assigning bandwidths");
	    bw.newLine();
	    bw.close();
	}
	catch (IOException e) {
  ...
  */
    ...
    class SeedTokenizer extends StreamTokenizer {
	protected SeedTokenizer(Reader r) {
	    super(r);
