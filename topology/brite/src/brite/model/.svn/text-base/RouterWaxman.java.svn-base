...
/**
 * RouterWaxman implements a model by Waxman on router level topology. ASWaxman
 * implements an analogous model on Autonomous System (AS) level topologies.
 * Please see the class ASWaxman for documentation. These models are very
 * similar on both the router and the AS level.
 */
public final class RouterWaxman extends RouterModel {

	float alpha, beta;

	public RouterWaxman(int N, int HS, int LS, int nodePlacement, int m,
			float alpha, float beta, int growthType, int bwDist, float bwMin,
			float bwMax) {
		this.N = N;
		this.alpha = alpha;
		this.beta = beta;
	...
	public String toString() {
		String modelParams = "Model (" + ModelConstants.RT_WAXMAN
				+ " - RTWaxman):  ";
	...
	private float getEdgeProb(Node src, Node dst) {
		...
		float L = (float) Math.sqrt(2.0) * HS;
		return (float) (alpha * Math.exp(-1.0 * (d / (beta * L))));
	}
	...
	public void ConnectNodes(Graph g) {
		...
		if (growthType == ModelConstants.GT_ALL) {
			...
			while (numNodesConnected < N) {
				...
				// Node src = (Node) g.getNodesVector().elementAt(rand1);
				Node src = nodesV[rand1]; // g.getKthNode(rand1);
				int numEdgesAdded = 0;

				while (numEdgesAdded < m || numNodesConnected < N) {
					int rand2 = (int) (Distribution
							.getUniformRandom(ConnectRandom) * N);
					// Node dst = (Node) g.getNodesVector().elementAt(rand2);
					// /*pick dst node randomly*/
					Node dst = nodesV[rand2];
					if (rand1 == rand2)
						continue; // no self loops
					if (g.hasEdge(src, dst))
						continue; // no multiedges
					// if (dst==null)
					// System.out.println("dst is null");
					float p = getEdgeProb(src, dst);
					float u = (float) Distribution
							.getUniformRandom(ConnectRandom);
					if (u < p) {
						...
						if (dst.getOutDegree() == 1)
							++numNodesConnected;
						++numEdgesAdded;
						//Util.DEBUG("added edge:("+src.getID()+", "+dst.getID()
						// +")  | numEdgesAdded="+numEdgesAdded+
						// " &numNodesConnected="+numNodesConnected);
					}
					// else System.out.println("missed (again)");
				}
			}
			...
			for (int i = m; i < N; ++i) {
				...
				while (numEdgesAdded < m) {
					if (src.getOutDegree() >= (N - m))
						break;

					int rand = (int) (Distribution
							.getUniformRandom(ConnectRandom) * i);
			...
			for (int i = 0; i < m; ++i) {
				...
				while (numEdgesAdded < m) {
					...
					int rand = m
							+ (int) (Distribution
									.getUniformRandom(ConnectRandom) * (float) (N - m)); /*
																						 * pick
																						 * rand
																						 * b
																						 * /
	...
	public Graph Generate() {
		// System.out.println("Generating " + N + " node graph with " +
		// nodePlacement + " node placement");
		Graph g = new Graph(N);
		super.PlaceNodes(g, ModelConstants.RT_NODE); /*
													 * Step1: Place Nodes on our
													 * plane
													 */
