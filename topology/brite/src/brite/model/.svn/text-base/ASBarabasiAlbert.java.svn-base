 ...
 */
public final class ASBarabasiAlbert extends ASModel {
     ...
     */
    public ASBarabasiAlbert(int N, int HS, int LS,  int nodePlacement, int m, int bwDist,
		      float bwMin, float bwMax){
	this.N = N;
  ...
  */
  public void ConnectNodes(Graph g) {
	...
	/*flip a coin*/
	float d = Distribution.getUniformRandom(ConnectRandom);
	
	/*determine "slot" where coin fell, that is our dest node */
	float last = 0;
	int dstI=0;
	for (dstI=0; i<nodesOutDeg.length; ++dstI){
	  last+=(float) nodesOutDeg[dstI]/sumOutDeg;  //this is how edge probability is computed
	  if (d<last)
	    break;
	}
