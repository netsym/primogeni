...
public final class RouterBarabasiAlbert extends RouterModel {
   ...
   public RouterBarabasiAlbert(int N, int HS, int LS,  int nodePlacement, int m, int bwDist,
	...
	String modelParams = "Model ("+ModelConstants.RT_BARABASI+" - RTBarabasi):  ";
	...
	for (int i=m+1; i<N; ++i) {
		...
		for (dstI=0; i<nodesOutDeg.length; ++dstI){
		  last+=(float) nodesOutDeg[dstI]/sumOutDeg;
		  if (d<last)
		    break;
		}
