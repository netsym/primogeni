...
public class TopDownHierModel extends Model {
	...
	public TopDownHierModel(ArrayList Models, int edgeConnMethod, int k,
				int bwInter, float interMin, float interMax, int bwIntra,
				float intraMin, float intraMax, double duplication_threshold) 
	{
		if (Models.size() < 2) {
		...
		this.duplication_threshold = duplication_threshold;

	}

	public String toString() {
		// TODO: include TD params also!
	...
	public void ConnectNodes(Node s, Node d, Graph gFlat, int direction) {
		...
		if (sFlat == null || dFlat == null)
			Util.ERR("NULL NODE found when connecting topologies. Crashing.");
	...
	public void ConnectTopologies(Graph g0, Graph gFlat) {
		...
		for (int i = 0; i < size; ++i) {
			...
			/* connect the routers of neighboring AS nodes according to edgeConn method*/																			
		}
	}
	...
	public Graph FlattenGraph(Graph g0) {
		...
		return new Graph(VectorOfGraphs);
		// return (FixCollisions(new Graph(VectorOfGraphs)));
	}

	public Graph Generate() {
		/* Step 1: Generate each level of topology */
		Util.MSG("Generating Top Level (AS) graph)");
		Graph g0 = ((Model) Models.get(0)).Generate(); // g0 is top-level graph (AS)
		...
		ArrayList<Topology> topos=new ArrayList<Topology>();
		Random r=new Random();
		int i;
		//modified here to change duplication rate
		//double duplication_threshold=0.4;
		int thres=(int)(vec.length*(1-duplication_threshold));
		if (thres<1) thres=1;
		
		for (i = 0; i < thres; ++i) {
			Util.MSG("Generating <regular> Router Topology # " + (i + 1) + " of"
					+ vec.length +"(real total="+thres+ ")...");
			topos.add(i,new Topology((Model) Models.get(1)));
		}
		for(;i<vec.length;i++) {
			Util.MSG("Generating <duplicate> Router Topology # " + (i + 1) + " of"
					+ vec.length +"(dupl total="+(vec.length-thres)+ ")...");
			topos.add(new Topology(topos.get(r.nextInt(thres))));
		}
		...
		for (i = 0; i < vec.length; ++i) {
			...
			if (n.getNodeConf() instanceof ASNodeConf) {
				 ...
				 */
				((ASNodeConf) n.getNodeConf()).setTopology(topos.get(i), n.getID());
		...
		//Analyzer=new Analyzer(gFlat,"analyzer_rep");
		//Analyzer.computeDegreeDistribution();
		//Analyzer.computeClusterCoefficient();
		//Analyzer.computeHopDistributionAndCharacteristicPathLength();
