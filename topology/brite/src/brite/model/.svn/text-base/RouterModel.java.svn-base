 ...
 * models that will ultimately generate router level topologies. In this
 * distribution of BRITE, two subclasses, RouterWaxman and RouterBarabasiAlbert
 * use the functionality provided in this class. RouterModel (this class)
 * implements the PlaceNodes() and AssignBW() functions for router level models.
 * If you are creating your own router model, you may use use the functionality
 * provided in this class, but you are free to derive your class directly from
 * Model.
 * 
...
public abstract class RouterModel extends Model {
	...
	// No constructor because "RouterModel" alone is meaningless. Use subclass
	// for
	// constructors.
	...
	/**
	 * Place nodes of nodeType (router nodes in this case) onto the plane
	 * according to the NodePlacement type and add them to our graph g. Does
	 * collision checking to ensure that two nodes in the plane can never have
	 * the same (x,y) coords.
	 */
			...
			while (totalNodes < N) {
				for (int i = 0; i < numSquares; ++i) {
					for (int j = 0; j < numSquares; ++j) {
						numNodesToPlace = (int) (Distribution.getParetoRandom(
								PlaceRandom, 1000000 * LS * LS, 1.0));
	...
	// this serves no function and is an implementation side affect. should
	// (can) never be called.*/
