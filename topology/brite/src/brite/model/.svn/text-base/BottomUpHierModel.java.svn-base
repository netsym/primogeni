...
public class BottomUpHierModel extends Model {
    ...
    int assignType;
    int bwInter;
    float interMax, interMin;
    Random AssignRandom;     // we get this when we need it from our RandomGenManager 
    Random GroupingRandom;
    
    public BottomUpHierModel(Model r, int numASNodes, int groupingMethod, int asType, int bwInter, float interMin, float interMax)
    {
	this.r = r;
  ...
  private int getNodesPerAS(int numGroupedSoFar) {
	...
	nodesPerAS = (int)(Distribution.getUniformRandom(AssignRandom) * (float)N);
	...
	for (int i=0; i<numASNodes; ++i) {
	  ...
	  for (int j = 0; j<nodesPerAS; ++j) {
	    int indexToRemove = (int) (Distribution.getUniformRandom(GroupingRandom) * (float) g0Nodes.size());
	...
	int startRand = (int) (Distribution.getUniformRandom(GroupingRandom)*(float)N);
