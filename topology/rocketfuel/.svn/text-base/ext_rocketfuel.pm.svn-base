...
my $relocate_file = "relocate.in";
my %relocate;
my $denoise_latlong_file = "denoise_latlong.in";
my %denoise_latlong;
...
sub get_router_level_map {
  ...
  read_relocate_file($relocate_file);
  ...
  foreach my $asnum (@aslist)  {
    ...
    if($loc_adjusted > 0) {
      ...
      print STDERR "rocketfuel: location adjusted: AS$asnum $loc_adjusted ($r%)\n";
    }
  }
...
sub debug {
  if (defined $routers{1221}{1799}) {
    print STDERR "### debug: @{$routers{1221}{1799}->{ipaddrs}}, $routers{1221}{1799}->{cutoff}\n";
  }
  if (defined $neighbors{1221}{1799}{1221}{1770}) {
    print STDERR "### debug: $neighbors{1221}{1799}{1221}{1770}->{latency}, $neighbors{1221}{1799}{1221}{1770}->{linktype}\n";
  }
  if (defined $neighbors{1221}{1770}{1221}{1799}) {
    print STDERR "### debug: $neighbors{1221}{1770}{1221}{1799}->{latency}, $neighbors{1221}{1770}{1221}{1799}->{linktype}\n";
  }
}
...
sub get_latencies {
  ...
  while(<IN_LAT>) {
    ...
    if(/([^\d]+)(\d+)\s+([^\d]+)(\d+)\s+(\d+)/) {
      ...
      if (defined $relocate{$rfrom}) {
        $rfrom = $relocate{$rfrom};
        if (!defined $already_relocated{$asnum}{$fromuid}) {
          if (defined $num_as_relocate{$asnum}) {
            $num_as_relocate{$asnum}++;
          } else { $num_as_relocate{$asnum} = 1; }
          $already_relocated{$asnum}{$fromuid} = 1;
        }
      }
      if (defined $relocate{$rto}) {
        $rto = $relocate{$rto};
        if (!defined $already_relocated{$asnum}{$touid}) {
          if (defined $num_as_relocate{$asnum}) {
            $num_as_relocate{$asnum}++;
          } else { $num_as_relocate{$asnum} = 1; }
          $already_relocated{$asnum}{$touid} = 1;
        }
      }
...
#        if ($rfrom eq "SAN JOSE,CA,US" and $rto eq "SAN JOSE,CA,US") {
#          print STDERR "### dist: $known_distances{$rfrom}{$rto}, num: $known_distances_num{$rfrom}{$rto}\n"
#        }
  ...
  foreach my $asnum (sort keys %neighbors) {
	...
	foreach my $nuid (sort keys %{$neighbors{$asnum}{$uid}{$nasnum}}) {
	  ...
	  if($asnum == $nasnum && $uid == $nuid) {
	    ...
	    if (!defined $touch{$asnum}{$uid}) {
	      $touch{$asnum}{$uid} = 1;
	    } else {
	      $touch{$asnum}{$uid}++;
	    }
#	    if ($asnum == 6461 and $uid == 496) {
#	      print "### rocketfuel: 6461:496, $touch{$asnum}{$uid}\n";	      
#	    }
		...
		if (!defined $touch{$nasnum}{$nuid}) {
		  $touch{$nasnum}{$nuid} = 1;
		} else {
		  $touch{$nasnum}{$nuid}++;
		}
#	    if ($nasnum == 6461 and $nuid == 496) {
#	      print "### rocketfuel: 6461:496, $touch{$nasnum}{$nuid}\n";	      
#	    }		
...
#            if ($asnum == $toasnum and $touid < 0) {
#              $tmp++;
#            }
        if ($touid >= 0) {
          if (defined $touch{$asnum}{$uid}) {
            $touch{$asnum}{$uid}--;
          } else { die "Error: the router '$asnum:$uid' never touched"; }
        }
        
#	    if ($asnum == 6461 and $uid == 496) {
#	      print "### rocketfuel: 6461:496, $routers{$asnum}{$uid}->{location}, $touch{$asnum}{$uid}\n";	      
#	    }        
	    $rml++;
	  } elsif($touid >= 0 &&
...
#	    }        
    ...
    foreach my $uid (keys %{$routers{$asnum}}) {
	 ...
	 !defined $routers{$asnum}{$uid}->{location} ||
	 $routers{$asnum}{$uid}->{location} =~ /\*,\s*\*/) {
#        if (defined $routers{$asnum}{$uid}->{location} ) {
#          print STDERR "### $routers{$asnum}{$uid}->{location}\n";
#        }
	delete $routers{$asnum}{$uid};
#	if (defined $touch{$asnum}{$uid} and $touch{$asnum}{$uid} <= 0) {
#	  print "### rocketfuel: \$touch{$asnum}{$uid} is $touch{$asnum}{$uid}\n";
#	}
	$rmn++;
...
#      print STDERR "### rocketfuel: remove access links: $tmp\n";
    }
  }
...
sub relocate {
  my $tot_num_relocate = 0;
  foreach my $asnum (keys %routers) {
    if (!defined $num_as_relocate{$asnum}) {
      $num_as_relocate{$asnum} = 0;
    }
    foreach my $uid (keys %{$routers{$asnum}}) {
      if (defined $routers{$asnum}{$uid}->{location} and
          defined $relocate{$routers{$asnum}{$uid}->{location}}) {
        $routers{$asnum}{$uid}->{location} = $relocate{$routers{$asnum}{$uid}->{location}};
        $num_as_relocate{$asnum}++;
      }
    }
    $tot_num_relocate += $num_as_relocate{$asnum};
    print STDERR "rocketfuel: relocated: AS$asnum $num_as_relocate{$asnum}\n";
  }
  print STDERR "rocketfuel: relocated: total $tot_num_relocate\n";

  foreach my $old (keys %relocate) {
    if (defined $latlong_lookup{$old}) {
      delete $latlong_lookup{$old};
    }
    $latlong_lookup{$relocate{$old}} = 0;
  }

}

sub read_relocate_file {
  my ($relocate_file) = @_;
  open IN_REL, "$relocate_file" || die "Can't open $relocate_file for reading: $!";
  while (<IN_REL>) {
    chomp;
    s/\#.*$//g;
    if (/^(.+,.+,.+)=>(.+,.+,.+)$/) {
...
sub fix_latencies {
  ...
  foreach my $asnum (sort { $a <=> $b } keys %routers) {
    foreach my $uid (keys %{$routers{$asnum}}) {
      if (defined $latlong_lookup{$routers{$asnum}{$uid}->{location}} ) {
        $routers{$asnum}{$uid}->{latlong} =
          $latlong_lookup{$routers{$asnum}{$uid}->{location}};
      } else { print STDERR "rocketfuel: fix_latencies: Can't retrieve latlong information for $asnum:$uid:($routers{$asnum}{$uid}->{location})\n"; }
    }
  }
...
#                if ($neighbors{$asnum}{$uid}{$nasnum}{$nuid}->{latency} >= 100) {
#                  print STDERR "### known: $neighbors{$asnum}{$uid}{$nasnum}{$nuid}->{latency} from $asnum:$uid($routers{$asnum}{$uid}->{location}) to $nasnum:$nuid($routers{$nasnum}{$nuid}->{location})\n";
#                }
...
#                if ($neighbors{$asnum}{$uid}{$nasnum}{$nuid}->{latency} >= 100) {
#                  print STDERR "### cal: $neighbors{$asnum}{$uid}{$nasnum}{$nuid}->{latency} from $asnum:$uid($routers{$asnum}{$uid}->{location},$routers{$asnum}{$uid}->{latlong}) to $nasnum:$nuid($routers{$nasnum}{$nuid}->{location},$routers{$nasnum}{$nuid}->{latlong})\n";
#              }
...
sub denoise_latlong {
  read_denoise_latlong($denoise_latlong_file);
  my $tot_num_denoise_latlong = 0;
  foreach my $asn (keys %routers) {
    my $num_denoise_latlong = 0;
    foreach my $uid (keys %{$routers{$asn}}) {
      if (defined $routers{$asn}{$uid}->{location} and
          defined $denoise_latlong{$routers{$asn}{$uid}->{location}} ) {
#        print STDERR "$routers{$asn}{$uid}->{location}\n";
#        print STDERR "$routers{$asn}{$uid}->{latlong}\n";
#        print STDERR "$asn:$uid\n";
        $routers{$asn}{$uid}->{latlong} = latlong_format("$denoise_latlong{$routers{$asn}{$uid}->{location}}");
        $num_denoise_latlong++;
#        print STDERR "$routers{$asn}{$uid}->{latlong}\n";
      }
    }
    $tot_num_denoise_latlong += $num_denoise_latlong;
    print STDERR "rocketfuel: denoise latlong: AS$asn $num_denoise_latlong\n";
  }
  print STDERR "rocketfuel: denoise latlong: total $tot_num_denoise_latlong\n";
...
sub latlong_format {
  ...
  my ($lat_degree, $lat_min) = split /\./, $lat;
  my ($lat_NS, $long_WE);
  my ($long_degree, $long_min) = split /\./, $long;
  if ($lat_degree >= 0) {
    $lat_NS = "N";
  } else {
    $lat_NS = "S";
  ...
  $lat_degree = abs $lat_degree;
  $long_degree = abs $long_degree;
  $lat_min = sprintf "%.0f", $lat_min*60/10000;
  $long_min = sprintf "%.0f", $long_min*60/10000;
  my $latlong_new = "$lat_degree:$lat_min$lat_NS,$long_degree:$long_min$long_WE";
}

sub read_denoise_latlong {
  my ($denoise_latlong_file) = @_;
  open IN_DELATLONG, "$denoise_latlong_file" || die "Can't open $denoise_latlong_file for reading: $!";
  while (<IN_DELATLONG>) {
    chomp;
    s/\#.*$//g;
    if (/^(.+,.+,.+)=>(.+,.+)$/) {
      my $loc = $1;
      my $latlong = $2;
      $denoise_latlong{$loc} = $latlong;
    }
  }
#  print STDERR %denoise_latlong;
#  print STDERR "\n";
  close(IN_DELATLONG);
}
...
#        foreach my $touid (keys %{$peering{$asnum}{$toasnum}{$uid}}) {
#          print STDERR "### $asnum:$uid<=>$toasnum:$touid\n";
#        }
