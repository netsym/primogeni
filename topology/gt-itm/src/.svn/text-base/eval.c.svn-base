	...
	if (vv->rank == 0) {
		...
		if (a) {
			...
			if (u == &dummy) {
			    if (verbose) {
			    if (artic_pt)
				printf(" and %ld (this ends a connected component of the graph)\n", idx(g, artic_pt));
			    else
				printf("Isolated vertex %ld\n", idx(g, v));
			    }
			    ...
			    if (artic_pt && verbose)
				printf(" and articulation point %ld\n", 
				idx(g, artic_pt));
			    t = active_stack;
			    active_stack = v->link;
			    if (verbose) printf("Bicomponent %ld", idx(g,v));
			    count++;
			    if (verbose) {
			    if (t == v)
				putchar('\n');
