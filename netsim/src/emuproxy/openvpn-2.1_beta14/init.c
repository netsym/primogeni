...
/*#define CONNECTION_PLUGIN_TUN_WRITE_FILENO 11*/

#define ARGVNUM 64
#define ARGVLEN 10240

void
start_ssfgwd(struct context *c)
{
  char* argv[ARGVNUM];
  char* arglist;
  int argc;
  ...
  argv[0] = getenv("SSFGWD_PATHNAME");
  if(!argv[0]) {
    fprintf(stderr, "In order for OpenVPN to run the emulation gateway, "
	    "the following environment variables should be defined:\n"
	    "  SSFGWD_PATHNAME\tPoints to the executable of ssfgwd (including the path)\n"
	    "  SSFGWD_ARGLIST\tList of additional command-line arguments to ssfgwd\n");
    exit(1);
  }
  argv[1] = "-a"; /* running as attached */

  argc = 2;
  arglist = getenv("SSFGWD_ARGLIST");
  if(arglist) {
    char mystr[ARGVLEN];
    char* word;
    strncpy(mystr, arglist, ARGVLEN-1); mystr[ARGVLEN-1] = 0;
    for(word = strtok(mystr, " "); word && argc<ARGVNUM-1; word = strtok(0, " "))
      argv[argc++] = word;
  }
  argv[argc] = 0;

  pipe(tt->pipe_vpn2gwd_fds);
  pipe(tt->pipe_gwd2vpn_fds);

  if ((tt->ssfgwd_pid = fork()) == 0) { /* child process */
    dup2(tt->pipe_vpn2gwd_fds[0], STDIN_FILENO);
    dup2(tt->pipe_gwd2vpn_fds[1], STDOUT_FILENO);
    close(tt->pipe_vpn2gwd_fds[0]);
    close(tt->pipe_vpn2gwd_fds[1]);
    close(tt->pipe_gwd2vpn_fds[0]);
    close(tt->pipe_gwd2vpn_fds[1]);

    execvp(argv[0], argv);
    perror("Cannot execute ssfgwd");
  } else {
    msg (M_INFO, "Spawning ssfgwd");
    setenv_int(c->c2.es, "connection_plugin_pipe_fileno", tt->pipe_vpn2gwd_fds[1]);
    /*dup2(tt->pipe_vpn2gwd_fds[1], CONNECTION_PLUGIN_TUN_WRITE_FILENO);*/
    close(tt->pipe_vpn2gwd_fds[0]);
    close(tt->pipe_gwd2vpn_fds[1]);
  }
}
...
void
stop_ssfgwd(struct context *c)
{
  struct tuntap *tt = c->c1.tuntap;
  if(tt->ssfgwd_pid > 0) {
    msg (M_INFO, "Killing ssfgwd");
    kill(tt->ssfgwd_pid, SIGKILL);
    tt->ssfgwd_pid = 0;
  }
}
#endif
...
#ifdef SSFNET_HIJACK_TUN
      if(c->options.mode == MODE_SERVER) start_ssfgwd(c);
#endif
...
#ifdef SSFNET_HIJACK_TUN
	  if(c->options.mode == MODE_SERVER) stop_ssfgwd(c);
#endif
