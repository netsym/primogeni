...
#ifdef WIN32
#error "SSFNET_HIJACK_TUN not implemented on this platform."
#endif
#include <unistd.h>
#endif
...
#ifdef SSFNET_HIJACK_TUN
  pid_t ssfgwd_pid;
  int pipe_vpn2gwd_fds[2];
  int pipe_gwd2vpn_fds[2];
#endif
...
pipe_read_handle (const struct tuntap *tt)
{
  return tt->pipe_gwd2vpn_fds[0];
}

static inline event_t
pipe_write_handle (const struct tuntap *tt)
{
  return tt->pipe_vpn2gwd_fds[1];
}
#endif
      ...
      if (!persistent || *persistent != rwflags)
	{
	  ...
	  if(tt->ssfgwd_pid > 0) {
	    if(rwflags & EVENT_READ)
	      event_ctl (es, pipe_read_handle (tt), EVENT_READ, arg);
	    if(rwflags & EVENT_WRITE)
	      event_ctl (es, pipe_write_handle (tt), EVENT_WRITE, arg);
	  } else event_ctl (es, tun_event_handle (tt), rwflags, arg);
#else
...
#ifdef SSFNET_HIJACK_TUN
void start_ssfgwd(struct context *c);
void stop_ssfgwd(struct context *c);
#endif
