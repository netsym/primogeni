...
static int safe_read_tun(struct tuntap* tt, uint8_t* buf, int bufsiz)
{
  int rcvd_len = 0;
  while(rcvd_len < bufsiz) {
    int ret = read_tun (tt, buf+rcvd_len, bufsiz-rcvd_len);
    if(ret < 0) return -1;
    rcvd_len += ret;
  }
  return rcvd_len;
...
read_incoming_tun (struct context *c)
{
  ...
  if(c->options.mode == MODE_SERVER) {
    int leadlen = safe_read_tun(c->c1.tuntap, (uint8_t*)&c->c2.to_link_dst, sizeof(in_addr_t));
    //c->c2.to_link_dst = ntohl(c->c2.to_link_dst);
    ASSERT(leadlen == sizeof(in_addr_t));
    /*struct in_addr x; x.s_addr = c->c2.to_link_dst;
      fprintf(stderr, ">>> (got=%d) src=%s\n", leadlen, inet_ntoa(x));*/

#ifdef TUN_PASS_BUFFER
#error "TUN_PASS_BUFFER under SSFNET_HIJACK_TUN not supported"
#endif

    ASSERT (buf_init (&c->c2.buf, FRAME_HEADROOM (&c->c2.frame)));
    ASSERT (buf_safe (&c->c2.buf, MAX_RW_SIZE_TUN (&c->c2.frame)));
    { int totlen;
      struct openvpn_iphdr* iph = (struct openvpn_iphdr*)BPTR(&c->c2.buf);
      c->c2.buf.len = safe_read_tun (c->c1.tuntap, (unsigned char*)iph, 20);
      ASSERT(c->c2.buf.len == 20);
      totlen = ntohs(iph->tot_len);
      /*fprintf(stderr, ">>> (got=%d) totlen=%d\n", c->c2.buf.len, totlen);*/
      ASSERT(totlen <= MAX_RW_SIZE_TUN (&c->c2.frame));
      c->c2.buf.len += safe_read_tun (c->c1.tuntap, ((unsigned char*)iph)+20, totlen-20);
      if(c->c2.buf.len != totlen) {
	fprintf(stderr, ">>> got=%d (including ip header), expected %d\n", c->c2.buf.len, totlen);
	struct in_addr x; x.s_addr = c->c2.to_link_dst;
	fprintf(stderr, "lead addr: %s\n", inet_ntoa(x));
	fprintf(stderr, "version_len=%x, ttl=%d, proto=%d\n", iph->version_len, iph->ttl, iph->protocol);
	ASSERT(c->c2.buf.len == totlen);
      }
    }
  } else {
#endif
