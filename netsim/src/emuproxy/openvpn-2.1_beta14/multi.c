  ...
  if (m->pending)
      ...
      if (BLEN (&c->c2.buf) > 0)
	{
	  ...
	  if (TUNNEL_TYPE (m->top.c1.tuntap) == DEV_TYPE_TUN)
		...
		{
#ifndef SSFNET_HIJACK_TUN
		  /* don't drop the packet if we run it as the
		     simulation gateway. -jason. */
		...
		{
#ifndef SSFNET_HIJACK_TUN
		  /* client-to-client mode is off, by force, if we run
		     it as the simulation gateway. -jason. */
      ...
      if (mroute_flags & MROUTE_EXTRACT_SUCCEEDED)
	{
	  ...
	  if (mroute_flags & (MROUTE_EXTRACT_BCAST|MROUTE_EXTRACT_MCAST))
...
#endif
	    {
	      ...
	      ASSERT(m->top.options.mode == MODE_SERVER);
	      ASSERT(dest.type == MR_ADDR_IPV4);
	      memcpy (dest.addr, &m->top.c2.to_link_dst, 4);
	      /*struct in_addr x; x.s_addr = m->top.c2.to_link_dst;
	      fprintf(stderr, "to link: %s\n", inet_ntoa(x));*/
#endif
