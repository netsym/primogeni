...
#ifdef SSFNET_HIJACK_TUN
      if(tt->ssfgwd_pid > 0)
	ret = writev(tt->pipe_vpn2gwd_fds[1], vect, 2);
      else ret = writev(tt->fd, vect, 2);
#else
...
#ifdef SSFNET_HIJACK_TUN
    if(tt->ssfgwd_pid > 0)
      return write (tt->pipe_vpn2gwd_fds[1], buf, len);
    else return write (tt->fd, buf, len);
#else
...
read_tun (struct tuntap* tt, uint8_t *buf, int len)
{
  ...
  if (tt->ipv6)
...
#ifdef SSFNET_HIJACK_TUN
    if(tt->ssfgwd_pid > 0)
      ret = readv(tt->pipe_gwd2vpn_fds[0], vect, 2);
    else ret = readv(tt->fd, vect, 2);
#else
    ...
    { int ret;
      if(tt->ssfgwd_pid > 0)
	ret = read (tt->pipe_gwd2vpn_fds[0], buf, len);
      else ret = read (tt->fd, buf, len);
      /*fprintf(stderr, "len=%d, ret=%d\n", len, ret);*/
      return ret;
    }
#else
...
#ifdef SSFNET_HIJACK_TUN
  if(tt->ssfgwd_pid > 0)
    return putmsg (tt->pipe_vpn2gwd_fds[1], NULL, &sbuf, 0) >= 0 ? sbuf.len : -1;
  else return putmsg (tt->fd, NULL, &sbuf, 0) >= 0 ? sbuf.len : -1;
#else
...
#ifdef SSFNET_HIJACK_TUN
  if(tt->ssfgwd_pid > 0)
    return getmsg (tt->pipe_gwd2vpn_fds[0], NULL, &sbuf, &f) >= 0 ? sbuf.len : -1;
  else return getmsg (tt->fd, NULL, &sbuf, &f) >= 0 ? sbuf.len : -1;
#else
...
#ifdef SSFNET_HIJACK_TUN
    if(tt->ssfgwd_pid > 0)
      return openbsd_modify_read_write_return (writev (tt->pipe_vpn2gwd_fds[1], iv, 2));
    else return openbsd_modify_read_write_return (writev (tt->fd, iv, 2));
#else
...
#ifdef SSFNET_HIJACK_TUN
    if(tt->ssfgwd_pid > 0) return write (tt->pipe_vpn2gwd_fds[1], buf, len);
    else return write (tt->fd, buf, len);
#else
    return write (tt->fd, buf, len);
#endif
...
#ifdef SSFNET_HIJACK_TUN
    if(tt->ssfgwd_pid > 0)
      return openbsd_modify_read_write_return (readv (tt->pipe_gwd2vpn_fds[0], iv, 2));
    else return openbsd_modify_read_write_return (readv (tt->fd, iv, 2));
#else
...
#ifdef SSFNET_HIJACK_TUN
    if(tt->ssfgwd_pid > 0) return read (tt->pipe_gwd2vpn_fds[0], buf, len);
...
#ifdef SSFNET_HIJACK_TUN
    if(tt->ssfgwd_pid > 0)
      return read (tt->pipe_gwd2vpn_fds[0], buf, len);
    else return read (tt->fd, buf, len);
#else
    return read (tt->fd, buf, len);
#endif
...
#ifdef SSFNET_HIJACK_TUN
      if(tt->ssfgwd_pid > 0)
	return freebsd_modify_read_write_return (writev (tt->pipe_vpn2gwd_fds[1], iv, 2));
      else return freebsd_modify_read_write_return (writev (tt->fd, iv, 2));
#else
...
#ifdef SSFNET_HIJACK_TUN
      if(tt->ssfgwd_pid > 0)
	return freebsd_modify_read_write_return (readv (tt->pipe_gwd2vpn_fds[0], iv, 2));
      else return freebsd_modify_read_write_return (readv (tt->fd, iv, 2));
#else
