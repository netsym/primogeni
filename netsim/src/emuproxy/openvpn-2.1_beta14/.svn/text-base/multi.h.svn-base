...
multi_process_outgoing_tun (struct multi_context *m, const unsigned int mpp_flags)
{
  ...
  {
    ASSERT(mi->context.options.mode == MODE_SERVER);
    ASSERT(mi->context.options.push_ifconfig_defined);
    uint32_t to_tun_src = htonl(mi->context.options.push_ifconfig_local);
    dmsg (D_TUN_RW, "TUN WRITE LEAD ADDRESS");
    int size = write_tun (mi->context.c1.tuntap, (uint8_t*)&to_tun_src, sizeof(uint32_t));
    check_status (size, "write lead addr to TUN/TAP", NULL, mi->context.c1.tuntap);
    if(size != sizeof(uint32_t))
      msg (D_LINK_ERRORS,
	   "TUN/TAP packet was destructively fragmented on write to %s (tried=%d,actual=%d)",
	   mi->context.c1.tuntap->actual_name, (int)sizeof(uint32_t), size);
  }
#endif
