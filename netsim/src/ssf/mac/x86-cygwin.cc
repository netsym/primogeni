/*
 * x86-cygwin :- machine support for x86/cygwin machine configuration.
 *
 * Supported machine configurations include:
 *   host_cpu={i?86},  host_os={cygwin*}
 *
 * Global variables are NOT shared; they're kept in the separate data
 * segment of each individual process created for each
 * processor. Application inadvertently using global variables may
 * find this feature rather confusing! So remember, there is no
 * shared global variables (such as those defined as global static).
 * The shared-memory heap is obtained from explicit mmap. Private 
 * variables are actually global variables within each process. 
 *
 * Mutual exclusion is implemented from atomic exchange.
 */

#ifdef PRIME_SSF_ARCH_X86_CYGWIN
...
#if SSF_USE_THREADS
  // the first thread creates the key for private data; the first
  // thread and all other threads subsequently spawned will use it to
...
void ssf_yield_proc() 
{
  ...
  sched_yield();
#else
#warning "WARNING: can't use sched_yield(), resulting inefficient use of ssf_yield_proc()"
  usleep(0);
#endif
}
...
#endif /*PRIME_SSF_ARCH_X86_CYGWIN*/
