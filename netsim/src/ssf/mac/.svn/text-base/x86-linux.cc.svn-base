/*
 * x86-linux :- hardware support for x86/Linux
 * machine configuration.
 *
 * Supported machine configurations include:
 *   host_cpu={i?86}, host_os={linux, pc_linux, gnu, pc_gnu}
 *
...
#ifdef PRIME_SSF_ARCH_X86_LINUX
...
void ssf_map_private_data(int pid, size_t size)
{
  ...
  if(!SSF_SDATA_ACTIVE) {
    assert(pid == 0); // if inactive, it must be one process only
#if PRIME_SSF_DEBUG
...
// xchg does atomic exchange; it's implemented in <asm/system.h> from
...
#else
static int linux_clone(void)
{
  // no idea if this is any better than fork()
  return syscall(__NR_clone, CLONE_FS|CLONE_FILES|SIGCHLD, (void*)0);
}
...
#endif /*PRIME_SSF_ARCH_X86_LINUX*/
