/*
 * x86-darwin :- support for Mac OS on Intel processors.
 */

#ifdef PRIME_SSF_ARCH_X86_DARWIN

#ifndef __PRIME_SSF_MACHINE_X86_DARWIN_H__
#define __PRIME_SSF_MACHINE_X86_DARWIN_H__

#ifndef __PRIME_SSF_MACHINES_H__
#error "ERROR: x86-darwin.h must not be included explicitly"
#endif

#include <stdio.h>

/* Here we use pthreads for multiprocessing. Darwin does not use GNU
   LD, so we have not found a method for multiple processes to share
   common data segment. When using pthreads, both data and heap are
   shared. However, we intentionally put heap segment as private so
   that our shared memory manager (arena) will be engaged to allow
   better performance. */
...
namespace ssf {
  ...
  inline void ssf_thread_delete(ssf_thread_type* tid) { pthread_cancel(*tid); }

  inline void ssf_thread_mutex_init(ssf_thread_mutex* lock) { pthread_mutex_init(lock, 0); }
  inline void ssf_thread_mutex_wait(ssf_thread_mutex* lock) { pthread_mutex_lock(lock); }
  inline void ssf_thread_mutex_signal(ssf_thread_mutex* lock) { pthread_mutex_unlock(lock); }
  inline int  ssf_thread_mutex_try(ssf_thread_mutex* lock) { return  !pthread_mutex_trylock(lock); }

  inline void ssf_thread_cond_init(ssf_thread_cond* cond) { pthread_cond_init(cond, 0); }
  inline void ssf_thread_cond_wait(ssf_thread_cond* cond, ssf_thread_mutex* lock) { pthread_cond_wait(cond, lock); }
  inline void ssf_thread_cond_signal(ssf_thread_cond* cond) { pthread_cond_signal(cond); }
  inline void ssf_thread_cond_broadcast(ssf_thread_cond* cond) { pthread_cond_broadcast(cond); }
...
#endif /*__PRIME_SSF_MACHINE_X86_DARWIN_H__*/
