/*
 * Doug Lea's memory allocator was modified to fit into SSF. We
 * consider the software as part of SSF and distributed as such, with
 * complete understanding of the terms and conditions of its original
 * copyright claims, as stated verbatim below.
 */
...
#define DL_PREFIX(x) dl##x
#else
#define DL_PREFIX(x) dl/**/x
#endif
#endif /*undef DL_PREFIX*/

#ifdef USE_DL_PREFIX
#define public_cALLOc    DL_PREFIX(calloc)
#define public_fREe      DL_PREFIX(free)
#define public_cFREe     DL_PREFIX(cfree)
#define public_mALLOc    DL_PREFIX(malloc)
#define public_mEMALIGn  DL_PREFIX(memalign)
#define public_rEALLOc   DL_PREFIX(realloc)
#define public_vALLOc    DL_PREFIX(valloc)
#define public_pVALLOc   DL_PREFIX(pvalloc)
#define public_mALLINFo  DL_PREFIX(mallinfo)
#define public_mALLOPt   DL_PREFIX(mallopt)
#define public_mTRIm     DL_PREFIX(malloc_trim)
#define public_mSTATs    DL_PREFIX(malloc_stats)
#define public_mUSABLe   DL_PREFIX(malloc_usable_size)
#define public_iCALLOc   DL_PREFIX(independent_calloc)
#define public_iCOMALLOc DL_PREFIX(independent_comalloc)
#else /*undef USE_DL_PREFIX*/
...
#    if defined(BSD) || defined(DGUX) || defined(HAVE_GETPAGESIZE)
       /*extern size_t getpagesize();*/ /*conflict with unistd definition on darwin. -jason.*/
...
/* changed by jason to make it signed integer */
#define NFASTBINS  ((int)fastbin_index(request2size(MAX_FAST_SIZE))+1)
...
// We shouldn't use global variable if used in SSF.
...
static void do_check_malloc_state()
{
  ...
  for (i = 0; i < NFASTBINS; ++i) {
    ...
    while (p != 0) {
      ...
      assert(fastbin_index(chunksize(p)) == (unsigned int)i); /* changed by jason for signed integer */
      p = p->fd;
    }
  }
  ...
  for (i = 1; i < NBINS; ++i) {
    ...
    for (p = last(b); p != b; p = p->bk) {
      ...
      if (i >= 2) {
        ...
        idx = bin_index(size);
        assert(idx == (unsigned int)i); /* changed by jason for signed integer */
        /* lists are sorted */
        assert(p->bk == b || 
