/**
 * \file new_tcp_seqwnd.h
 * \brief Definition of the TCPSeqWindow class.
 */

#ifndef __NEW_TCP_SEQWND_H__
#define __NEW_TCP_SEQWND_H__
...
class TCPSeqWindow {
 public:
	...
	{
		//printf("starting sequence configured=%d\n", start_seqno);
	}
	...
	/*uint32 firstUnused() {
	  //printf("in FIRSTUNUSED: start_seqno=%d used_size=%d\n", (int)start_seqno, (int)used_size);
	  return start_seqno + used_size;
    }*/
	...
	void setStart(const uint32 seqno) {
		//printf("setstart called with %d\n", seqno);
		start_seqno = seqno;
		//printf("\n**************** SEQNO was initialized to %d\n, start_seqno");
	}
	...
	uint32 expect() {
	  //printf("in EXPECT: start_seqno=%d syn_included=%d fin_included=%d\n",
		//	  (int)start_seqno, (int)syn_included, (int)fin_included);
		return start_seqno+syn_included+fin_included;
	}
 ...
 protected:
	...
	void seq_shift(uint32 offset) {
		//printf("seq_shift called with %d\n", offset);
		start_seqno += offset;
	}
