...
static u32 htcp_cwnd_undo(struct sock *sk)
{
	...
	struct htcp *ca = (struct htcp *) inet_csk_ca(sk);
...
{
	const struct inet_connection_sock *icsk = (struct inet_connection_sock *) inet_csk(sk);
	const struct tcp_sock *tp = (struct tcp_sock *) tcp_sk(sk);
	struct htcp *ca = (struct htcp *) inet_csk_ca(sk);
...
{
	const struct tcp_sock *tp = (struct tcp_sock *) tcp_sk(sk);
	const struct htcp *ca = (struct htcp *) inet_csk_ca(sk);
...
static void htcp_state(struct sock *sk, u8 new_state)
{
	...
	case TCP_CA_Loss:
		htcp_reset((struct htcp *) inet_csk_ca(sk));
		break;
	}
}
