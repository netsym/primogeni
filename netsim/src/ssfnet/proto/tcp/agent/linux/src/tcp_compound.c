...
//extern int abs (int __x) __THROW __attribute__ ((__const__)) __wur; <-- causing error on mac
...
static inline void cvegas_enable(struct sock *sk)
{
	...
	struct compound *vegas = (struct compound *) inet_csk_ca(sk);
...
static u32 qroot(u64 a)
{
       ...
       } while ((x1 - x) > 1 || (x - x1) > 1); // (abs(x1 - x) > 1);
...
{
	const struct compound *ca = (struct compound *) inet_csk_ca(sk);
