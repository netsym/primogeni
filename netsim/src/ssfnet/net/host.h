...
public:
	SSFNET_PROPMAP_DECL(
		SSFNET_CONFIG_PROPERTY_DECL(int, host_seed)
		SSFNET_CONFIG_PROPERTY_DECL(float, time_skew)
		SSFNET_CONFIG_PROPERTY_DECL(VirtualTime, time_offset)
		SSFNET_CONFIG_CHILDREN_DECL_SHARED(Interface, nics)
	)
	SSFNET_STATEMAP_DECL(
		SSFNET_CONFIG_STATE_DECL(float, traffic_intensity)
		IP2IFACE_MAP ip2iface;
		IPAddress default_ip;
		HostTimerQueue * tmr_queue;
		SSFNET_INT2PTR_MAP * tmr_map;
		int tmr_handle;
		prime::rng::Random * rng;
		int rng_seed;
		PortalIfaceTable * portals;
		SSFNET_CONFIG_CHILDREN_DECL_UNSHARED(Host,Interface,nics)
	)
	SSFNET_CONFIG_CHILDREN_DECL(Interface, nics)
	SSFNET_ENTITY_SETUP(&(unshared.nics) )
