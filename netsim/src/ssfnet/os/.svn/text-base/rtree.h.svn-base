...
I found this at: http://www.superliminal.com/sources/sources.htm
TITLE
...
10 Jan 2010
o Nate Changed the search function
o Nate Changed the code to work within ssfnet
o Nate Changed the insert/delete/
*/
...
///        This version uses new/delete for nodes, I recommend using a fixed size ator for efficiency.
///
...
inline ELEMTYPE& RectMin(ELEMTYPE& l, ELEMTYPE& r) {
	if( l < r)return l;
	return r;
}
template<class ELEMTYPE>
inline ELEMTYPE& RectMax(ELEMTYPE& l, ELEMTYPE& r) {
	if( l > r)return l;
	return r;
}
...
public:
  ...
  long XXX_total_memory;

  class SearchResult {
  public:
    virtual ~SearchResult() {}
	  virtual bool addResult(DATATYPE& result)=0;
  };
  ...
  void Insert(Rect& r, const DATATYPE& a_dataId);
  ...
  void Remove(Rect& r, const DATATYPE& a_dataId);
  ...
  void Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], SearchResult& rv);
  void Search(Rect& r, SearchResult& rv);
...
protected:
  ...
  bool Search(Node* a_node, Rect* a_rect, SearchResult& rv);
...
RTREE_TEMPLATE
void RTREE_QUAL::Insert(Rect& r, const DATATYPE& a_dataId)
{
	InsertRect(&r, a_dataId, &m_root, 0);
}
...
RTREE_TEMPLATE
void RTREE_QUAL::Remove(Rect& r, const DATATYPE& a_dataId)
{
	RemoveRect(&r, a_dataId, &m_root);
}
...
RTREE_TEMPLATE
void RTREE_QUAL::Search(Rect& r, SearchResult& rv)
{
	  Search(m_root, &r, rv);
}

RTREE_TEMPLATE
void RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], SearchResult& rv)
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
...
{
	XXX_total_memory+=sizeof(ListNode);
#ifdef RTREE_DONT_USE_MEMPOOLS
  return new ListNode;
#else // RTREE_DONT_USE_MEMPOOLS
  // EXAMPLE
...
typename RTREE_QUAL::Rect RTREE_QUAL::CombineRect(Rect* a_rectA, Rect* a_rectB)
{
  ...
  for(int index = 0; index < NUMDIMS; ++index)
  {
    newRect.m_min[index] = RectMin(a_rectA->m_min[index], a_rectB->m_min[index]);
    newRect.m_max[index] = RectMax(a_rectA->m_max[index], a_rectB->m_max[index]);
  }
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, SearchResult& rv)
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  if(a_node->IsInternalNode()) // This is an internal node in the tree
  {
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        if(!Search(a_node->m_branch[index].m_child, a_rect, rv))
        {
        	return true; // Don't continue searching
        }
      }
    }
  }
  else // This is a leaf node
  {
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
    	  if(!rv.addResult(a_node->m_branch[index].m_data)) {
