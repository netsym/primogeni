...
AttrNode* AttrNode::search_attribute(const char* keypath, AttrNode* root)
{
...
void AttrNode::find_attributes(const char* keypath, std::vector<AttrNode*>& collected, 
                               int onematch, AttrNode* root)
{
  ...
  if(*keypath == 0) goto clear_and_return; // hit the end
    ...
    // adjust the key path for the next level
    const char* newkeypath = keypath+strlen(mykey);
