...
#define YY_INPUT(buf, result, max_size) \
  ...
  else { prime::dml::lex_line_start = (char*)buf; result = strlen(buf); \
    prime::dml::lex_line_pos += prime::dml::lex_line_length; \
    prime::dml::lex_line_length = result; }
...
#define PRIME_DML_SET_LOCALE  yylval.locinfo = prime::dml::lex_location()
#define PRIME_DML_MARK_LOCALE prime::dml::lex_location(yylval.keyvalue->locinfo)
#else
...
<CMT>.     ;
<CMT>\n    { ++prime::dml::lex_lineno; BEGIN NORM; }
...
<NORM>{KEYNAME}  { yylval.keyvalue = new prime::dml::KeyValue(yytext);
                   PRIME_DML_MARK_LOCALE;
                   PRIME_DML_PRINT_KEYVAL(LX_IDENT);
                   return LX_IDENT; }

<NORM>{STRING}  { yylval.keyvalue = new prime::dml::KeyValue(yytext);
                  PRIME_DML_MARK_LOCALE;
                  PRIME_DML_PRINT_KEYVAL(LX_STRING);
                  return LX_STRING; }

<NORM>\n  { ++prime::dml::lex_lineno; }
<NORM>.   ;
