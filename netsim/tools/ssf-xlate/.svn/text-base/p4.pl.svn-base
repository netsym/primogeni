...
# recognized wait statements: semWait is not included here.
...
sub define_procedure {
  ...
  foreach $thestr (@lines) {
	...
	$str = substr($str, $pos+4); # pass the current wait statement (the first 4 letters)
      ...
      while(($pos=has_keyword('semWait', $str)) >= 0) {
	$nlabels++;
  ...
  print OUTFILE "class _ssf_procedure_${clsnam}_${fctnam} : public prime::ssf::ssf_procedure \{ ";
  ...
  if($isaction) {
    print OUTFILE "_ssf_procedure_${clsnam}_${fctnam}(prime::ssf::Process* _ssf_focus) : ";
    if($pretty_prt) { print OUTFILE "\n    "; }
    print OUTFILE "prime::ssf::ssf_procedure(_ssf_focus) {} ";
    if($pretty_prt) { print OUTFILE "\n"; }
  } else {
    print OUTFILE "_ssf_procedure_${clsnam}_${fctnam}(prime::ssf::ssf_procedure_container* _ssf_focus, ";
    @list = @arglist;
    while(@list) {
      my($arg_type) = shift @list;
      my($arg_name) = shift @list;
    ...
    if($pretty_prt) { print OUTFILE "\n    "; }
    print OUTFILE "prime::ssf::ssf_procedure((void (prime::ssf::ssf_procedure_container::*)(prime::ssf::Process*))";
  ...
  if($clsnam ne '') {
    print OUTFILE "prime::ssf::ssf_procedure* ${clsnam}::_ssf_create_procedure_${fctnam}(";
  } else { print OUTFILE "prime::ssf::ssf_procedure* _ssf_create_procedure_${fctnam}("; }
  if($isaction) {
    print OUTFILE ") { ";
    if($pretty_prt) { print OUTFILE "\n  "; }
    print OUTFILE "return new _ssf_procedure_${clsnam}_action(this); ";
  } else {
    @list = @arglist;
    while(@list) {
      my($arg_type) = shift @list;
  ...
  else { print OUTFILE "(prime::ssf::Process* _ssf_proc) { "; }
  if($pretty_prt) { print OUTFILE "\n  "; }
  if($isaction) {
    print OUTFILE "prime::ssf::Process* _ssf_proc = prime::ssf::current_process(); ";
    if($pretty_prt) { print OUTFILE "\n  "; }
  }
  print OUTFILE "if(_ssf_proc->isSimple()) ";
  if($pretty_prt) { print OUTFILE "\n    "; }
  print OUTFILE "prime::ssf::ssf_throw_exception(prime::ssf::ssf_exception::other_exception, \"simple process, but ";
  if($clsnam ne '') {
    print OUTFILE "non-simple procedure: ${clsnam}::${fctnam}\"); ";
  } else {
    print OUTFILE "non-simple procedure: ${fctnam}\"); ";
  }
  if($pretty_prt) { print OUTFILE "\n  "; }
  ...
  if($isaction || scalar(@arglist) == 2 &&
     ($arglist[0] =~ m/^\s*SSF_Process\s*\*\s*/ ||
      $arglist[0] =~ m/^\s*Process\s*\*\s*/ ||
      $arglist[0] =~ m/^\s*process\s*\*\s*/)) {
    print OUTFILE "if(!_ssf_proc->_proc_top_stack()) { ";
    if($pretty_prt) { print OUTFILE "\n    "; }
    print OUTFILE "_ssf_proc->_proc_push_stack(_ssf_create_procedure_${fctnam}";
    if($isaction) { print OUTFILE "()); "; }
    ...
    if($pretty_prt) { print OUTFILE "\n    "; }
    if($clsnam ne '') {
      print OUTFILE "prime::ssf::ssf_throw_exception(prime::ssf::ssf_exception::other_exception, \"${clsnam}::${fctnam} not a starting procedure\"); ";
    } else {
      print OUTFILE "prime::ssf::ssf_throw_exception(prime::ssf::ssf_exception::other_exception, \"${fctnam} not a starting procedure\"); ";
    }
  }
  if($pretty_prt) { print OUTFILE "\n  "; }
  print OUTFILE "\} ";
  ...
  if($clsnam ne '') {
    print OUTFILE "_ssf_proc->_proc_is_proper(\"${clsnam}::${fctnam}\"); ";
  } else {
    print OUTFILE "_ssf_proc->_proc_is_proper(\"${fctnam}\"); ";
  }
  if($pretty_prt) { print OUTFILE "\n  "; }

  # _ssf_pframe is the current procedure frame on stack
  print OUTFILE "_ssf_procedure_${clsnam}_${fctnam}* _ssf_pframe = ";
  print OUTFILE "(_ssf_procedure_${clsnam}_${fctnam}*)_ssf_proc->_proc_top_stack(); ";
  if($pretty_prt) { print OUTFILE "\n  "; }

  # jump table
  print OUTFILE "switch(_ssf_pframe->_proc_entry) { ";
  if($pretty_prt) { print OUTFILE "\n  "; }
  my($label);
  ...
  for($offset=0; $offset<=$#lines; $offset++, $linenum++) {
	...
	$zzz .= "{ _ssf_pframe->_proc_entry = ${label}; ";
	$zzz .= "${refs}${wait}(${args}); ";
	$zzz .= "if(_ssf_pframe->call_suspended()) { return; _ssf_sync${label}: ; ";
	if($lhs ne '') {
	  $zzz .= "${lhs} = ${refs}_proc_is_timedout(); ";
	}
	$zzz .= "} }";
	$label++;
    ...
    $open_idx = has_keyword('semWait', $thestr);
    if($open_idx >= 0) {
      my($zzz) = '';
      while($open_idx >= 0) {
	my($preidx) = rindex($thestr, ';', $open_idx);
	...
	$args =~ s/\s//g;
	if($args ne '') {
	  die "file ${filename} line ${linenum}: semWait does not have argument\n";
	}

	$zzz .= "{ if(${prefix}semValue()>0) ${prefix}semWait(); ";
	$zzz .= "else { _ssf_pframe->_proc_entry = ${label}; ";
	$zzz .= "${prefix}semWait(); return; ";
	$zzz .= "_ssf_sync${label}: ; } }";
	$label++;
	...
	$pos--;
      ...
      $thestr  = "{ _ssf_pframe->_proc_entry = ${label}; ";
      if($lhs ne '') {
	if($args_clobber ne '') {
...
sub declare_procedure {
  ...
  print OUTFILE "prime::ssf::ssf_procedure* _ssf_create_procedure_${fctnam}(";
  if($isaction) { print OUTFILE "); "; }
  else {
    my @list = @arglist;
    while(@list) {
    ...
    print OUTFILE "void* retaddr = 0)${reststr}";
  }
  if($pretty_prt) { print OUTFILE "\n"; }
  ...
  if($isaction) {
    ...
    print OUTFILE "void ${fctnam}(prime::ssf::Process*)";
    $reststr =~ s/\s//g;
    if($reststr eq "=0;") {
...
if(scalar(@ARGV) == 1) {
  $pretty_prt = 0; $filename = shift @ARGV;
} elsif(scalar(@ARGV) == 2) {
  my $myarg = shift @ARGV;
  if($myarg eq '-x') {
    $pretty_prt = 1; $filename = shift @ARGV;
  } else { die "Usage: $0 [-x] filename\n"; }
} else { die "Usage: $0 [-x] filename\n"; }
...
while(!eof(SSFFILE)) {
  ...
  if($one_line =~ m/^#\s+(\d+)\s+"([^"]+)"/ ||
    ...
    if($F =~ m/_tmp[0-9]+_.cc/) {
      ...
      $line_number = $L;
      $filename = $F;
      print $one_line; # the line may contain extra information
  ...
  if($inst_pos >= 0) { # if the line contains instrumentation
    ...
    if($inst_mark =~ m/SSF PROCEDURE/ && $inst_mark !~ m/SIMPLE/) {
      # //! SSF PROCEDURE
